...
namespace CDP.AdoNet.Repositories
{
    public class WarehouseRepositoryConnected : IRepository<Warehouse>
    {
        ...
        public void Create(Warehouse obj)
        {
            ...
            {
                command.CommandText =
                    "INSERT dbo.Warehouse (Id, City, State) VALUES (@Id, @City, @State)";
        ...
        public void Update(Warehouse obj)
        {
            ...
            {
                command.CommandText =
                    "UPDATE dbo.Warehouse SET City = @City, State = @State WHERE Id = @Id";
                SetParameters(command, obj);
                command.ExecuteNonQuery();
            }
        }
        ...
        public void Delete(int id)
        {
            using (var command = _transactionWrapper.CreateCommand())
            {
                command.CommandText = "DELETE FROM dbo.RouteOfCargo WHERE OriginWarehouseId = @Id " +
        ...
        private void SetParameters(IDbCommand command, Warehouse obj)
        {
            command.AddParameter("@Id", obj.Id);
            command.AddParameter("@City", obj.City);
            command.AddParameter("@State", obj.State);
        }
    }
}